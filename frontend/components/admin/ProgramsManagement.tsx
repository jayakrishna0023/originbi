      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 text-xs sm:text-sm text-brand-text-light-secondary dark:text-brand-text-secondary pt-6 pb-2">
        ...
        <div className="flex justify-center w-full sm:w-1/3 order-1 sm:order-2">
          <div className="flex items-center gap-2">
            ...
            {/* Simple Pagination Numbers (for now, simplistic logic like Registration) */}
            {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
              // A simplified version of pagination logic or reuse the complex one if preferred.
              // For brevity, let's use a simpler range or just show First/Last/Current
              // Reusing logic from RegistrationManagement would be ideal but complex to inline.
              // Let's implement a basic window around current page.
              let page = i + 1;
