...
export class CorporateService {
    ...
    async findAll(
        ...
        try {
            ...
            throw new InternalServerErrorException(
                `Failed to fetch corporate registrations`,
            );
        }
    }
    ...
    async update(id: number, dto: any) {
        ...
        return this.dataSource.transaction(async (manager) => {
            ...
            if (
                ...
                if (
                    dto.mobile &&
                    (dto.mobile !== account.mobileNumber ||
                        dto.countryCode !== account.countryCode)
                ) {
                    // Check mobile uniqueness (excluding current account)
                    const existingMobile = await manager
                        .getRepository(CorporateAccount)
                        .findOne({
                            where: {
                    ...
                    if (existingMobile && existingMobile.id !== account.id) {
                        throw new BadRequestException(
                            'Mobile number already exists for another corporate account',
                        );
                    }
                }
                ...
                if (dto.employeeCode && dto.employeeCode !== account.employeeRefId) {
                    ...
                    if (existingRef && existingRef.id !== account.id) {
                        throw new BadRequestException(
                            `Employee Ref ID '${dto.employeeCode}' is already in use.`,
                        );
                    }
                }
            ...
            if (dto.businessLocations)
                account.businessLocations = dto.businessLocations;
    ...
    async updateCredits(id: number, credits: number) {
        ...
        // Calculate delta
        // const oldCredits = account.totalCredits;
    ...
    async topUpCredits(id: number, amount: number, reason?: string) {
        ...
        return this.dataSource.transaction(async (manager) => {
            ...
            return {
                success: true,
                newAvailable: account.availableCredits,
                newTotal: account.totalCredits,
            };
        });
    }
    ...
    async create(dto: CreateCorporateRegistrationDto) {
        ...
        if (existingUser) {
            this.logger.warn(
                `Failed to create corporate user: Email '${email}' already exists. UserID: ${existingUser.id}`,
            );
        ...
        try {
            ...
            this.logger.error(
                `Database Transaction Failed in Corporate Create: ${dbError.message}`,
