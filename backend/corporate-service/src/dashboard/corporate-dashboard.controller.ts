...
export class CorporateDashboardController {
    ...
    async createOrder(
        ...
        if (!email) throw new BadRequestException('Email is required');
        if (!creditCount || creditCount <= 0) throw new BadRequestException('Valid credit count is required');
    ...
    async verifyPayment(
        ...
        if (!email) throw new BadRequestException('Email is required');
        if (!paymentDetails) throw new BadRequestException('Payment details are required');
